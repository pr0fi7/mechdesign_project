...
# class MD_Calculation:
#     class SymbolInfo:
#         __slots__ = ['name','description','comment','def_value','range']
#         def __init__(self,descr,**kwargs):
#             self.description = descr
#             self.comment = kwargs['comment'] if 'comment' in kwargs.keys() else None
#             self.def_value = kwargs['def_value'] if 'def_value' in kwargs.keys() else None
#             self.range = kwargs['range'] if 'range' in kwargs.keys() else None
        
#     def __init__(self):
#         # all calcuations will hold a dictionary. 
#         #   key = name of variable
#         #   value = item of class SymbolInfo, the description field is mandatory
#         self.info = dict()
        
#     def add_symbol_to_info(self,name,descr,**kwargs):
#         self.info[name] = self.SymbolInfo(descr, **kwargs)
    
    
#     def print_equation(self,var_name):
#         my_expression = self.getattr(var_name)
#         display(my_expression)
...
def myHelp(Expression):
    ...
    for t in Expression.free_symbols: 
        ...
        if not t.def_value==None: print('    '+' '*len(t.name)+'     '+str(t.def_value))
